<template>
  <div class="tree_page">
      <div class="btn" v-show="treeData && treeData.length>0">
          <div @click="allSelect" v-show="organizingType!='1598190201723793410'" v-fastClick>全选</div>
...
export default {
  name: "",
  props:{
    // 组织类型, 若为再生造粒1598190201723793410,只能勾选一个菜单
    organizingType:{
      type: String,
      default: '',
    },
  ...
  watch: {
    tree:{
      handler(newVal){
        ...
        // this.checkedMenu = this.menuList
        // this.checkedBtn = this.btnList
      },
      // deep: false,
      // immediate:false
    },
  ...
  mounted(){
    this.$bus.on("appletParams",(treeData,btnList,menuList)=>{
  ...
  beforeDestroy(){
    this.$bus.off("appletParams")
  },
  methods: {
    // 数据存储
    handleStore(){
      this.$bus.$emit("res_appletTreeData",this.checkedMenu,this.checkedBtn)
    },
    ...
    checkChange(data,status,sonStatus){
      ...
      // 组织类型, 若为再生造粒1598190201723793410,只能勾选一个菜单
      // 若是选中,先清空,再选择当前菜单
      if (this.organizingType && this.organizingType=='1598190201723793410' && status) {
        ...
        this.checkedMenu = []
        this.checkedMenu.push(data.menuNo)
        this.$refs.tree.setCheckedNodes(this.checkedMenu)
      }else{
