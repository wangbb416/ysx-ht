...
<template>
  <div>
    ...
    <MyTable
      ...
      :pageSize="pageSize"
      :isIndexBoo="true"
      :tableConfig="publicityeducationTabel"
    >
...
import { publicityeducationTabel } from "./config";
import {
  publicityeducationList,
  publicityeducationAdd,
  publicityeducationDelete,
  downloadpublicityeducation,
  releasePublicityeducation
...
export default {
  ...
  methods: {
    ...
    openDialog(type, data = null) {
      switch (type) {
        ...
        case "del":
          this.$confirm("确认删除？")
            .then(_ => {
              publicityeducationDelete({ id: data.id })
                .then(res => {
    ...
    downLoad() {
      ...
      // 导出
      exportXlsx("宣传教育", params, downloadpublicityeducation);
    },
    ...
    releaseManage(type, data) {
      ...
      releasePublicityeducation(params)
        .then(res => {
          if (res != -1) {
            this.dialogVisible = false;
            this.$message.success("提交成功");
