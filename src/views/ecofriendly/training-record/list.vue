...
<template>
  <div>
    ...
    <MyTable :data="tableData" :pageNum="pageNum" :pageSize="pageSize" :isIndexBoo="true"
             :tableConfig="trainingrecordTabel">
...
import {trainingrecordTabel} from "./config";
import {trainingrecordList, downloadtrainingrecord} from "@/api/ecofriendly";
  ...
  methods: {
    ...
    openDialog(type, data) {
      switch (type) {
        case'detail':
          this.$router.push({
            path: "/ysx/training-record-detail",
            query: {id: data.id}
          });
          break;
        default:
    ...
    downLoad() {
      ...
      // 导出
      exportXlsx('培训记录', params, downloadtrainingrecord);
