<!-- 摄像头管理 -->
<template>
  <div class="cameraManage_page" id="cameraManage_page">
    ...
    <div class="main">
      <div class="search" id="search">
        ...
        <div class="search-bt">
          ...
          <div class="button-blue" @click="handleOpenDialog('add')" v-if="MenuBtnValid('/cameraManage','新增')">新增</div>
        </div>
      </div>
      <el-table
        :data="tableData"
        ...
        <el-table-column fixed="right" label="操作" width="180">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="small"
              v-if="MenuBtnValid('/cameraManage','编辑')"
            >
              编辑
            </el-button>
            <el-button
              v-if="scope.row.status == 0 && MenuBtnValid('/cameraManage','启用/禁用')"
            ...
            <el-button
              ...
              type="text"
              size="small"
              v-if="MenuBtnValid('/cameraManage','删除')"
            >
              删除
            </el-button>
          </template>
      ...
      <div class="dialog_content">
        ...
        >
          <el-form-item label="设备名称" prop="name">
            <el-input
              :disabled="!isEdit && addData.id != null"
              placeholder="请输入设备名称"
          ...
          <el-form-item label="设备序列号" prop="serialNumber">
            <el-input
              :disabled="!isEdit && addData.id != null"
              placeholder="请输入设备序列号"
              v-model.trim="addData.serialNumber"
              onkeyup="this.value=this.value.replace(/[, ]/g,'')"
              clearable
              :maxlength="20"
            ></el-input>
          ...
          <el-form-item label="功能">
            <el-checkbox v-model="addData.remoteOpen">支持远程开启</el-checkbox>
            <el-checkbox v-model="addData.location">支持定位传输</el-checkbox>
            <!-- <el-checkbox-group v-model="addData.optionalFeatures">
              <el-checkbox :label="1" key="1">支持远程开启</el-checkbox>
              <el-checkbox :label="2" key="2">支持定位传输</el-checkbox>
            </el-checkbox-group> -->
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  data() {
    return {
      tabelHeigth: 420,
      breadData: [{name: "装备管理", path: ""}, {name: "摄像头管理"}],
      formData: {
        search: null,
        deviceModel: null,
        status: null
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      this.tabelHeigth =
        document.querySelector('#cameraManage_page').offsetHeight -
        257 -
        document.querySelector('#search').offsetHeight
    })
    window.onresize = () => {
      if (
        document.querySelector('#cameraManage_page') &&
        document.querySelector('#cameraManage_page').offsetHeight &&
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.tableData = []
      this.$http(this.GLOBALApi.cameraGetByPage(), params)
        .then(res => {
          if (res.code === 200 && res.data) {
            this.loading = false
    ...
    closeDialogVisible() {
      this.$nextTick(() => {
        ...
        this.addData.remoteOpen = false
        this.addData.location = false
      })
    ...
    // 查看详情
    getDetail(item) {
      this.$http(this.GLOBALApi.cameraDetail(), { no: item.deviceNo })
        .then(res => {
          if (res.code === 200 && res.data) {
            let data = res.data
            this.$nextTick(() => {
              ...
              this.addData.remoteOpen = data.remoteOpen === 0 ? true : false
              this.addData.location = data.location === 0 ? true : false
    ...
    handleOperateDialog(value, item) {
      ...
      switch (value) {
        case 'disable':
          url = this.GLOBALApi.cameraDisable()
          operateSuccess = '禁用成功'
          ...
          break
        case 'enable':
          url = this.GLOBALApi.cameraEnable()
          operateSuccess = '启用成功'
          this.dealBtn(url, params, operateSuccess)
          break
        case 'delate':
          url = this.GLOBALApi.cameraDelete()
          operateSuccess = '删除成功'
    ...
    handleSubmit() {
      this.$refs.addData.validate(valid => {
        if (valid) {
          if (this.isEdit) {
            this.$confirm('确认修改此设备吗?', '修改确认').then(() => {
              let data = {
                deviceNo: this.addData.id,
                deviceCode: this.addData.serialNumber,
                name: this.addData.name,
                deviceModel: this.addData.deviceModel,
                remoteOpen: this.addData.remoteOpen ? 0 : 1, //远程开启 0开启 1关闭
                location: this.addData.location ? 0 : 1 //定位 0开启 1关闭
              }
              this.$http(this.GLOBALApi.cameraEdit(), data)
                .then(res => {
                  if (res.code === 200 && res.data) {
    ...
    addDataFun() {
      ...
      let params = {
        name: this.addData.name, //设备名称
        deviceCode: this.addData.serialNumber, // 设备序列号
        deviceModel: this.addData.deviceModel, //设备型号
        remoteOpen: this.addData.remoteOpen ? 0 : 1, //远程开启 0开启 1关闭
        location: this.addData.location ? 0 : 1 //定位 0开启 1关闭
      }
      this.$http(this.GLOBALApi.cameraAdd(), params)
        .then(res => {
          if (res.code === 200 && res.data) {
