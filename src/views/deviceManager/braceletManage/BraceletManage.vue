<!-- 手环吊坠管理 -->
<template>
  <div class="braceletManage_page" id="braceletManage_page">
    ...
    <div class="main">
      <div class="search" id="search">
        ...
        <div class="search-bt">
          ...
          <div class="button-blue" @click="handleOpenDialog('add')" v-if="MenuBtnValid('/braceletManage','新增')">新增</div>
        </div>
      </div>
      <el-table :data="tableData" style="width: 100%" :height="tabelHeigth" v-loading="loading"
        ...
        <el-table-column fixed="right" label="操作" width="180">
          <template slot-scope="scope">
            ...
            v-if="MenuBtnValid('/braceletManage','新增')"
            >编辑
            </el-button>
            <el-button v-if="scope.row.status ==0 && MenuBtnValid('/braceletManage','启用/禁用')" @click="handleOperateDialog('disable',scope.row)" type="text" size="small"
            >禁用
            </el-button>
            <el-button v-if="scope.row.status !=0 && MenuBtnValid('/braceletManage','启用/禁用')" @click="handleOperateDialog('enable',scope.row)" type="text" size="small"
            >启用
            </el-button>
...
export default {
  ...
  data() {
    return {
      tabelHeigth: 420,
      breadData: [{name: "装备管理", path: ""}, {name: "手环吊坠管理"}],
      formData: {
        search: null,
        deviceModel: null,
        status: null,
  ...
  mounted() {
    let self = this;
    this.$nextTick(() => {
      this.tabelHeigth = document.querySelector("#braceletManage_page").offsetHeight - 257 - document.querySelector("#search").offsetHeight;
    });
    window.onresize = () => {
      if (document.querySelector("#braceletManage_page") && document.querySelector("#braceletManage_page").offsetHeight && document.querySelector("#search") && document.querySelector("#search").offsetHeight) {
        self.tabelHeigth = document.querySelector("#braceletManage_page").offsetHeight - 257 - document.querySelector("#search").offsetHeight;
      }
    };
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.tableData = []
      this.$http(this.GLOBALApi.braceletGetByPage(), params)
          .then((res) => {
            if (res.code === 200 && res.data) {
              this.loading = false;
    ...
    handleOpenDialog(type, val) {
      ...
      if (type == 'add') {
        ...
        // 查看详情
    getDetail(item) {
      this.$http(this.GLOBALApi.braceletDetail(),{no:item.deviceNo})
          .then((res) => {
            if (res.code === 200 && res.data) {
              let data = res.data
              this.$nextTick(() => {
    ...
    handleOperateDialog(value,item) {
      ...
      switch (value) {
        case 'disable':
          url = this.GLOBALApi.braceletDisable()
          operateSuccess = "禁用成功"
          ...
          break;
        case 'enable':
          url = this.GLOBALApi.braceletEnable()
          operateSuccess = "启用成功"
          this.dealBtn(url,params,operateSuccess);
          break;
        case 'delate':
          url = this.GLOBALApi.braceletDelete()
          operateSuccess = "删除成功"
        ...
        if (valid) {
          ...
          if (this.isEdit) {
            ...
            .then(()=>{
              ...
              this.$http(this.GLOBALApi.braceletEdit(), data)
              .then((res) => {
                if (res.code === 200 && res.data) {
    ...
    addDataFun() {
      ...
      this.$http(this.GLOBALApi.braceletAdd(), params)
        .then((res) => {
          if (res.code === 200 && res.data) {
