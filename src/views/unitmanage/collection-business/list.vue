...
<template>
  <div>
    ...
    <div class="select-list">
      <div class="select-list-l">
        <MySearch
          ...
          :form="form"
          @search="getCollectCompaniesReset"
        ></MySearch>
      </div>
    ...
    <MyTable
      ...
      :tableConfig="collectCompaniesTable"
    >
    ...
    <MyPagination
      ...
      :size="pageSize"
      :total="total"
      @pageChange="getCollectCompanies"
      :hidePagination="true"
    ></MyPagination>
  </div>
</template>
...
import { collectCompaniesTable } from "./config";
import {
  collectCompaniesList,
  collectCompaniesDelete,
...
export default {
  ...
  methods: {
    ...
    exportTableAction() {
      let t = dateGrain();
      let str = `收集企业信息${t.dateYear}-${t.dateMonth}-${t.dateDay}`;
      let params = {
        companyType: 3,
        page: this.pageNum,
        size: this.pageSize,
    ...
    downLoadModule() {
      ...
      exportXlsx("收集企业信息导入模板", params, downloadFishingBoatList);
    },
    getCollectCompaniesReset() {
      this.pageNum = 1;
      this.getCollectCompanies();
    },
    // 获取
    getCollectCompanies(data) {
    ...
    openDialog(type, itemId = null) {
      switch (type) {
        case "add":
          this.$router.push({
            path: "/ysx/collection-business-add"
          });
          break;
        case "del":
          this.$confirm("本收集企业相关的一切内容将被删除，是否继续？", "提示")
            .then(_ => {
              collectCompaniesDelete(itemId)
                .then(res => {
                  if (res != -1) {
        ...
        case "detail":
          this.$router.push({
            path: "/ysx/collection-business-detail",
            query: { itemId: itemId }
          });
          break;
        default:
