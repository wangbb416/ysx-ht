...
import { disposalBusinessAdd } from "@/api/unitManagement";
...
export default {
  ...
  data() {
    return {
      titleList: [
        { name: "单位管理", path: "" },
        { name: "处置企业列表", path: "/ysx/disposal-business" },
        { name: "添加处置企业", path: "" }
      ],
      // 基本信息
  ...
  methods: {
    ...
    checkIconStatus() {
      if (this.formValue.companyName.trim() == "") {
        ...
        disposalBusinessAdd(obj)
          .then(res => {
            if (res != -1) {
              this.$router.push({
                path: `/ysx/disposal-business`
              });
