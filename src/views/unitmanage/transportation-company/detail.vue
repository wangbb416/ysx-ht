      ...
      <template #data_licensePictures="{ data }">
        <div class="pic_cell">
          <img
            ...
            @click="openDialog('openPic', data.licensePictures)"
...
import {
  ...
  transportationCompanyEdit,
  carrierInformation
} from "@/api/unitManagement";
import { addressRequest, uploadAction } from "@/api/other";
import { carrierInformationTable } from "./config";
import { CompanyType, carryType } from "@/utils/enum";
...
export default {
  ...
  data() {
    return {
      titleList: [
        { name: "单位管理", path: "" },
        { name: "运输企业列表", path: "/ysx/transportation-company" },
        { name: "运输企业详情", path: "" }
      ],
      itemId: "",
      // 表格
  ...
  methods: {
    getDetail(itemId) {
      transportationCompanyDetail(itemId)
        .then(res => {
          if (res != -1) {
    ...
    checkIconStatus() {
      if (this.checkBaseMsgStatus) {
        if (this.formValue.companyName.trim() == "") {
          ...
          transportationCompanyEdit(this.itemId, obj)
            .then(res => {
              if (res != -1) {
    ...
    openDialog(type, picUrl) {
      switch (type) {
        case "openPic":
          this.dialogVisible = true;
          this.licensePictures = picUrl;
          break;
        case "close":
