...
<template>
  <div>
    ...
    <div class="select-list">
      <div class="select-list-l">
        <MySearch
          ...
          :form="form"
          @search="getTransportationCompanyListReset"
        ></MySearch>
      </div>
    ...
    <MyTable
      ...
      :tableConfig="transportationCompanyTable"
    >
    ...
    <MyPagination
      ...
      :size="pageSize"
      :total="total"
      @pageChange="getTransportationCompanyList"
      :hidePagination="true"
    ></MyPagination>
  </div>
</template>
...
import { transportationCompanyTable } from "./config";
import {
  transportationCompanyList,
  transportationCompanyDelete,
...
export default {
  name: "transportation-company-list",
  components: {
    MyTable,
    MyPagination,
    MyBread,
  ...
  methods: {
        ...
        importExcel(form)
          .then(res => {
            if (res != -1) {
              ...
              that.getTransportationCompanyListReset()
            }
          })
          .catch(error => {
    ...
    exportTableAction() {
      let t = dateGrain();
      let str = `运输企业信息${t.dateYear}-${t.dateMonth}-${t.dateDay}`;
      let params = {
        companyType: 4,
        page: this.pageNum,
        size: this.pageSize,
    ...
    downLoadModule() {
      ...
      exportXlsx("运输企业信息导入模板", params, downloadFishingBoatList);
    },
    getTransportationCompanyListReset() {
      this.pageNum = 1;
      this.getTransportationCompanyList();
    },
    getTransportationCompanyList(data) {
      ...
      transportationCompanyList(params)
        .then(res => {
          if (res != -1) {
    ...
    openDialog(type, itemId = null) {
      switch (type) {
        case "add":
          this.$router.push({
            path: "/ysx/transportation-company-add"
          });
          break;
        case "del":
          this.$confirm("本运输企业相关的一切内容将被删除，是否继续？", "提示")
            .then(_ => {
              transportationCompanyDelete(itemId)
                .then(res => {
                  if (res != -1) {
        ...
        case "detail":
          this.$router.push({
            path: "/ysx/transportation-company-detail",
            query: { itemId: itemId }
          });
          break;
        default:
