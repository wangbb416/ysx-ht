...
<template>
  <div>
    ...
    <MyTable :data="tableData" :pageNum="pageNum" :pageSize="pageSize" :isIndexBoo="true" :tableConfig="favorablefishingpolicyTabel">
...
import {favorablefishingpolicyTabel} from "./config";
import {
  favorablefishingpolicyList,
  downloadFavorablefishingpolicy,
  checkFavorablefishingpolicyAdd,
  favorablefishingpolicyDelete,
} from "@/api/policydirect";
...
export default {
  ...
  methods: {
    ...
    getListData(data) {
      ...
      favorablefishingpolicyList(params)
        .then(res => {
          if (res != -1) {
    ...
    openDialog(type, data) {
      switch (type) {
        ...
        case'del':
          this.$confirm("确认删除？")
            .then(_ => {
              favorablefishingpolicyDelete({id: data.id})
                .then(res => {
                  console.log(res);
                  if (res != -1) {
    ...
    downLoad() {
      ...
      // 导出
      exportXlsx('惠渔政策', params, downloadFavorablefishingpolicy);
    },
    ...
    submit() {
      ...
      checkFavorablefishingpolicyAdd(params)
        .then(res => {
          if (res != -1) {
