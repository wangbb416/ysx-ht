...
<template>
  <div>
    ...
    <MyTable :data="tableData" :pageNum="pageNum" :pageSize="pageSize" :isIndexBoo="true" :tableConfig="learningrecordTabel">
      ...
      <template #slotEndTime="{ data }">
        <div>{{ data.endTime ?  getFormatTime(data.endTime) : '' }}</div>
      </template>
...
import {learningrecordTabel} from "./config";
import {learningrecordList, downloadlearningrecord} from "@/api/safeautonomy";
  ...
  methods: {
    ...
    openDialog(type, data) {
      switch (type) {
        case'detail':
          this.$router.push({
            path: "/ysx/learning-record-detail",
            query: {id: data.id}
          });
          break;
        default:
    ...
    downLoad() {
      ...
      // 导出
      exportXlsx('学习记录', params, downloadlearningrecord);
