...
import { safelearningTabel } from "./config";
import {
  safelearningList,
  checkSafelearningAdd,
  downloadSafelearning,
  releaseSafelearningList,
  safelearningListDelete
...
export default {
  ...
  methods: {
    ...
    openDialog(type, data = null) {
      switch (type) {
        ...
        case "del":
          this.$confirm("确认删除？")
            .then(_ => {
              safelearningListDelete({ id: data.id })
                .then(res => {
    ...
    downLoad() {
      ...
      // 导出
      exportXlsx("安全学习", params, downloadSafelearning);
    },
    ...
    releaseManage(type, data) {
      ...
      releaseSafelearningList(params)
        .then(res => {
          if (res != -1) {
            this.dialogVisible = false;
            this.$message.success("提交成功");
