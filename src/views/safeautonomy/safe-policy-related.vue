...
<template>
  <div>
    ...
    <MyTable :data="tableData" :pageNum="pageNum" :pageSize="pageSize" :isIndexBoo="true" :tableConfig="safepolicyrelatedTabel">
...
import {safepolicyrelatedTabel} from "./config";
import {
  safepolicyrelatedList,
  checkSafepolicyrelatedAdd,
  safepolicyrelatedDelete,
  downloadSafepolicyrelated
} from "@/api/safeautonomy";
...
export default {
  ...
  methods: {
    ...
    openDialog(type, data) {
      switch (type) {
        ...
        case'del':
          this.$confirm("确认删除？")
            .then(_ => {
              safepolicyrelatedDelete({id: data.id})
                .then(res => {
                  console.log(res);
                  if (res != -1) {
    ...
    downLoad() {
      ...
      // 导出
      exportXlsx('政策相关', params, downloadSafepolicyrelated);
    },
    ...
    submit() {
      ...
      checkSafepolicyrelatedAdd(params)
        .then(res => {
          if (res != -1) {
